...
# ── Constants ─────────────────────────────────────────────────────────────
MAX_BATCH_SIZE = 10
TEMP_STORAGE = {}  # Simple temp storage {file_id: (image_data, timestamp)}
...
def get_image_info(img: Image.Image) -> str:
    ...
    # Get color count for small images
    colors = "N/A"
    if img.width * img.height < 50000:
        colors = len(img.getcolors(maxcolors=256)) if img.getcolors(maxcolors=256) else ">256"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Watermark text
    elif context.user_data.get("waiting_watermark"):
        context.user_data["waiting_watermark"] = False
        file_id = context.user_data.get("file_id")
        if not file_id:
            await update.message.reply_text("❌ Image expired. Send it again.")
    ...
    # Percent resize
    elif context.user_data.get("waiting_percent"):
        try:
            percent = int(text)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "do_batch_start":
        ...
        await q.edit_message_text(
            ...
            f"Send up to {MAX_BATCH_SIZE} images.\n"
    ...
    if d == "do_remove_exif":
        img = await get_image(context, file_id)
        # Save without EXIF by creating new image
    ...
    # ── Resize Presets ──
    elif d.startswith("do_resize_") and d not in ["do_resize_custom"]:
        parts = d.split("_")
...
if __name__ == "__main__":
    ...
    print("🚀 Advanced Bot is running with 20+ features!")
    app.run_polling()
