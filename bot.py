import os, io, re, zipfile, csv, requests
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup
from telegram.ext import (
    ApplicationBuilder, CommandHandler, MessageHandler,
    CallbackQueryHandler, filters, ContextTypes,
)
from PIL import Image, ImageFilter, ImageEnhance, ImageDraw, ImageFont
if not hasattr(Image, 'ANTIALIAS'):
    Image.ANTIALIAS = Image.LANCZOS

import img2pdf, yt_dlp
from moviepy.editor import VideoFileClip
from pypdf import PdfReader, PdfWriter

TOKEN = os.environ.get("TOKEN")
MAX_BATCH = 10

# ── Optional libs ─────────────────────────────────────────
try:
    import qrcode; QR_OK = True
except ImportError:
    QR_OK = False
try:
    import pytesseract; OCR_OK = True
except ImportError:
    OCR_OK = False
try:
    from rembg import remove as rembg_remove; REMBG_OK = True
except ImportError:
    REMBG_OK = False
try:
    import pdfplumber; PLUMBER_OK = True
except ImportError:
    PLUMBER_OK = False

# ═══════════════════════════════════════════════════════════
#  STATES
# ═══════════════════════════════════════════════════════════
class S:
    COMPRESS_KB    = "compress_kb"
    INCREASE_KB    = "increase_kb"
    CUSTOM_SIZE    = "custom_size"
    CUSTOM_RATIO   = "custom_ratio"
    PERCENT        = "percent"
    IMG_WM         = "img_wm"
    VIDEO_GIF      = "video_gif"
    PDF_MERGE      = "pdf_merge"
    PDF_ENCRYPT    = "pdf_encrypt"
    PDF_WM         = "pdf_wm"
    QR_TEXT        = "qr_text"
    COLLAGE        = "collage_mode"
    BATCH_SIZE     = "batch_size"
    BATCH_KB       = "batch_kb"

def state(ctx): return ctx.user_data.get("st")
def set_state(ctx, s): ctx.user_data["st"] = s
def clear_state(ctx): ctx.user_data["st"] = None

# ═══════════════════════════════════════════════════════════
#  IMAGE HELPERS
# ═══════════════════════════════════════════════════════════
def compress_to_kb(img, target_kb):
    img = img.convert("RGB")
    lo, hi = 1, 95
    buf = io.BytesIO()
    for _ in range(14):
        mid = (lo + hi) // 2
        buf = io.BytesIO()
        img.save(buf, "JPEG", quality=mid, optimize=True)
        (lo := mid) if buf.tell() <= target_kb * 1024 else (hi := mid)
    buf.seek(0); return buf

def increase_to_kb(img, target_kb):
    img = img.convert("RGB")
    scale = 1.0
    for _ in range(25):
        w, h = int(img.width * scale), int(img.height * scale)
        r = img.resize((w, h), Image.LANCZOS)
        buf = io.BytesIO(); r.save(buf, "PNG")
        if buf.tell() / 1024 >= target_kb:
            buf.seek(0); return buf
        scale *= 1.15
    buf.seek(0); return buf

def apply_watermark(img, text, opacity=140):
    img = img.copy().convert("RGBA")
    layer = Image.new("RGBA", img.size, (255,255,255,0))
    draw = ImageDraw.Draw(layer)
    font = None
    for p in ["/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf",
               "/System/Library/Fonts/Helvetica.ttc"]:
        if os.path.exists(p):
            try: font = ImageFont.truetype(p, max(20, int(img.height*0.05))); break
            except: pass
    if not font: font = ImageFont.load_default()
    bb = draw.textbbox((0,0), text, font=font)
    tw, th = bb[2]-bb[0], bb[3]-bb[1]
    draw.text((img.width-tw-30, img.height-th-30), text, fill=(255,255,255,opacity), font=font)
    return Image.alpha_composite(img, layer).convert("RGB")

async def get_img(ctx, fid):
    f = await ctx.bot.get_file(fid)
    return Image.open(io.BytesIO(await f.download_as_bytearray()))

async def get_bytes(ctx, fid):
    f = await ctx.bot.get_file(fid)
    return bytes(await f.download_as_bytearray())

def img_buf(img, fmt="PNG", quality=95):
    buf = io.BytesIO()
    if fmt == "JPG": img = img.convert("RGB"); img.save(buf, "JPEG", quality=quality)
    else: img.save(buf, fmt, quality=quality)
    buf.seek(0); return buf

def make_collage(images, mode="grid"):
    TARGET = 900; PAD = 8
    imgs = [i.convert("RGB") for i in images]; n = len(imgs)
    if mode == "horizontal":
        H = TARGET // 2
        strips = [i.resize((int(i.width * H/i.height), H), Image.LANCZOS) for i in imgs]
        W = sum(s.width for s in strips) + PAD*(n-1)
        c = Image.new("RGB", (W, H), (230,230,230))
        x = 0
        for s in strips: c.paste(s, (x,0)); x += s.width + PAD
    elif mode == "vertical":
        W = TARGET // 2
        strips = [i.resize((W, int(i.height * W/i.width)), Image.LANCZOS) for i in imgs]
        H = sum(s.height for s in strips) + PAD*(n-1)
        c = Image.new("RGB", (W, H), (230,230,230))
        y = 0
        for s in strips: c.paste(s, (0,y)); y += s.height + PAD
    else:
        cols = 2; rows = (n+1)//2; cell = TARGET//cols
        c = Image.new("RGB", (cols*cell+PAD*(cols-1), rows*cell+PAD*(rows-1)), (230,230,230))
        for i, im in enumerate(imgs):
            im = im.resize((cell,cell), Image.LANCZOS)
            c.paste(im, ((i%cols)*(cell+PAD), (i//cols)*(cell+PAD)))
    buf = io.BytesIO(); c.save(buf, "JPEG", quality=92); buf.seek(0); return buf

CROP_PRESETS = {"square":(1,1),"story":(9,16),"post":(4,5),"wide":(16,9),"twitter":(2,1)}
def crop_ratio(img, preset):
    rw, rh = CROP_PRESETS.get(preset,(1,1))
    ow, oh = img.size; tr = rw/rh; or_ = ow/oh
    if or_ > tr:
        nw = int(oh*tr); l=(ow-nw)//2; return img.crop((l,0,l+nw,oh))
    nh = int(ow/tr); t=(oh-nh)//2; return img.crop((0,t,ow,t+nh))

# ═══════════════════════════════════════════════════════════
#  PDF HELPERS
# ═══════════════════════════════════════════════════════════
def compress_pdf(raw: bytes) -> io.BytesIO:
    """
    Re-encode PDF: copy pages through a fresh writer which strips
    unused objects, then compress every content stream.
    """
    reader = PdfReader(io.BytesIO(raw))
    writer = PdfWriter()
    writer.append(reader)                    # clones all pages + structure
    for page in writer.pages:
        try: page.compress_content_streams() # deflate each page stream
        except Exception: pass
    buf = io.BytesIO()
    writer.write(buf)
    buf.seek(0)
    return buf

def merge_pdfs(byte_list):
    w = PdfWriter()
    for b in byte_list:
        w.append(PdfReader(io.BytesIO(b)))
    buf = io.BytesIO(); w.write(buf); buf.seek(0); return buf

def split_pdf(raw):
    r = PdfReader(io.BytesIO(raw)); pages = []
    for i, pg in enumerate(r.pages):
        w = PdfWriter(); w.add_page(pg)
        buf = io.BytesIO(); w.write(buf); buf.seek(0)
        pages.append((i+1, buf))
    return pages

def encrypt_pdf(raw, pw):
    r = PdfReader(io.BytesIO(raw)); w = PdfWriter()
    w.append(r); w.encrypt(pw)
    buf = io.BytesIO(); w.write(buf); buf.seek(0); return buf

def rotate_pdf(raw, angle):
    r = PdfReader(io.BytesIO(raw)); w = PdfWriter()
    for pg in r.pages: pg.rotate(angle); w.add_page(pg)
    buf = io.BytesIO(); w.write(buf); buf.seek(0); return buf

def watermark_pdf(raw, text):
    try:
        from reportlab.pdfgen import canvas as rc
        r = PdfReader(io.BytesIO(raw)); w = PdfWriter()
        for pg in r.pages:
            wbuf = io.BytesIO()
            pw, ph = float(pg.mediabox.width), float(pg.mediabox.height)
            c = rc.Canvas(wbuf, pagesize=(pw,ph))
            c.setFont("Helvetica-Bold", 42)
            c.setFillColorRGB(0.7,0.7,0.7,alpha=0.35)
            c.translate(pw/2, ph/2); c.rotate(45)
            c.drawCentredString(0,0,text); c.save()
            wbuf.seek(0)
            pg.merge_page(PdfReader(wbuf).pages[0]); w.add_page(pg)
        buf = io.BytesIO(); w.write(buf); buf.seek(0); return buf
    except ImportError:
        return io.BytesIO(raw)

def pdf_info(raw):
    r = PdfReader(io.BytesIO(raw)); m = r.metadata or {}
    return (f"📄 *PDF Info*\n"
            f"Pages: {len(r.pages)}\n"
            f"Size: {len(raw)//1024} KB\n"
            f"Title: {m.get('/Title','—')}\n"
            f"Author: {m.get('/Author','—')}\n"
            f"Encrypted: {'Yes' if r.is_encrypted else 'No'}")

def extract_pdf_text(raw):
    if PLUMBER_OK:
        parts=[]
        with pdfplumber.open(io.BytesIO(raw)) as pdf:
            for i,pg in enumerate(pdf.pages,1):
                parts.append(f"─── Page {i} ───\n{pg.extract_text() or ''}")
        return "\n\n".join(parts)
    r = PdfReader(io.BytesIO(raw))
    return "\n\n".join(f"─── Page {i+1} ───\n{pg.extract_text() or ''}" for i,pg in enumerate(r.pages))

def extract_pdf_tables(raw):
    if not PLUMBER_OK: raise RuntimeError("pdfplumber not installed")
    out = io.StringIO(); w = csv.writer(out)
    with pdfplumber.open(io.BytesIO(raw)) as pdf:
        for i,pg in enumerate(pdf.pages,1):
            for j,tbl in enumerate(pg.extract_tables(),1):
                w.writerow([f"=== Page {i} Table {j} ==="])
                for row in tbl: w.writerow([c or "" for c in row])
                w.writerow([])
    buf = io.BytesIO(out.getvalue().encode()); buf.seek(0); return buf

# ═══════════════════════════════════════════════════════════
#  SOCIAL MEDIA DOWNLOAD
# ═══════════════════════════════════════════════════════════
SM_PATTERNS = [r"pinterest\.com/pin/",r"pin\.it/",r"instagram\.com/p/",
               r"tiktok\.com/@.*/video/",r"youtube\.com/shorts/",r"youtu\.be/",
               r"twitter\.com/.*/status/",r"x\.com/.*/status/"]

def is_sm_link(t): return any(re.search(p,t.lower()) for p in SM_PATTERNS)

async def dl_media(url):
    opts={"outtmpl":"downloads/%(title)s.%(ext)s","quiet":True,"no_warnings":True,
          "user_agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
    try:
        with yt_dlp.YoutubeDL(opts) as y:
            info=y.extract_info(url,download=True)
            return {"path":y.prepare_filename(info),"title":info.get("title","media"),
                    "filesize":info.get("filesize",0)}
    except Exception as e:
        if "pinterest" in url.lower():
            return await _pinterest_dl(url)
        raise e

async def _pinterest_dl(url):
    h={"User-Agent":"Mozilla/5.0"}
    r=requests.get(url,headers=h,timeout=15); html=r.text
    for pat,ext,key in [(r'"videoUrl":"(https:[^"]+)"',"mp4","pinterest_video"),
                        (r'"imageUrl":"(https:[^"]+)"',"jpg","pinterest_image")]:
        m=re.search(pat,html)
        if m:
            mu=m.group(1).replace("\\/","/"); rr=requests.get(mu,headers=h,stream=True,timeout=30)
            fp=f"downloads/{key}.{ext}"
            with open(fp,"wb") as f:
                for chunk in rr.iter_content(8192): f.write(chunk)
            return {"path":fp,"title":key.replace("_"," ").title(),"filesize":os.path.getsize(fp)}
    raise Exception("Could not extract media from Pinterest page")

async def video_to_gif(vp, gp, fps=10, dur=8):
    clip=VideoFileClip(vp)
    if clip.duration>dur: clip=clip.subclip(0,dur)
    if clip.w>480: clip=clip.resize(width=480)
    clip.write_gif(gp,fps=fps,logger=None); clip.close()

# ═══════════════════════════════════════════════════════════
#  MENUS  ── flat, sectioned, 3-wide where possible
# ═══════════════════════════════════════════════════════════
def R(*btns):
    """Make a keyboard row from (label, cb) pairs."""
    return [InlineKeyboardButton(l, callback_data=c) for l,c in btns]

def KB(rows): return InlineKeyboardMarkup(rows)

BACK = ("⬅ Back","menu_main")

def image_menu():
    return KB([
        R(("── 🖼  IMAGE TOOLS ──────────────","noop")),
        R(("📐 Resize","menu_img_resize"),("🔄 Convert","menu_img_convert"),("🗜 Compress","menu_img_compress")),
        R(("📈 Increase size","menu_img_increase"),("✂️ Crop","menu_img_crop"),("🔄 Rotate/Flip","menu_img_rotate")),
        R(("🎨 Effects","menu_img_effects"),("💧 Watermark","do_wm_prompt"),("📦 Batch","menu_batch")),
        R(("🖼 Collage","menu_collage"),("🖼 Remove BG","do_remove_bg"),("🔍 OCR→text","do_ocr")),
        R(("📷 EXIF viewer","do_exif"),("🗑 Strip EXIF","do_remove_exif"),("🌈 Auto color","do_auto_color")),
        R(("📏 Add border","do_add_border"),("ℹ️ Image info","do_img_info")),
        R(("── 📄  PDF TOOLS ────────────────","noop")),
        R(("🗜 Compress PDF","do_pdf_compress"),("📝 Extract text","do_pdf_text"),("📊 Tables→CSV","do_pdf_tables")),
        R(("📑 PDF→JPG","do_pdf2jpg"),("🖼 Image→PDF","do_img2pdf"),("🔗 Merge PDFs","do_pdf_merge_start")),
        R(("✂️ Split PDF","do_pdf_split"),("🔒 Encrypt","do_pdf_encrypt"),("🔓 Decrypt","do_pdf_decrypt")),
        R(("🔄 Rotate pages","menu_pdf_rotate"),("💧 Watermark PDF","do_pdf_wm_prompt"),("ℹ️ PDF info","do_pdf_info")),
        R(("── ⚡  OTHER ────────────────────","noop")),
        R(("🎬 Video→GIF","do_video_gif_prompt"),("📥 Download link","do_dl_prompt"),("🔲 QR code","do_qr_prompt")),
    ])

def resize_menu():
    return KB([
        R(("HD 1280×720","do_resize_1280_720"),("FHD 1920×1080","do_resize_1920_1080"),("4K 3840×2160","do_resize_3840_2160")),
        R(("640×480","do_resize_640_480"),("Thumb 256×256","do_resize_256_256"),("Square 1080×1080","do_resize_1080_1080")),
        R(("✏️ Custom size","do_resize_custom"),("📐 Custom ratio","do_ratio_custom"),("🎯 By %","do_percent_prompt")),
        R(BACK),
    ])

def convert_menu():
    return KB([
        R(("→ PNG","do_convert_PNG"),("→ JPG","do_convert_JPG"),("→ WEBP","do_convert_WEBP"),("→ PDF","do_img2pdf")),
        R(BACK),
    ])

def compress_menu():
    return KB([
        R(("50 KB","do_compress_50"),("100 KB","do_compress_100"),("200 KB","do_compress_200"),("500 KB","do_compress_500")),
        R(("1 MB","do_compress_1000"),("✏️ Custom KB","do_compress_custom")),
        R(BACK),
    ])

def increase_menu():
    return KB([
        R(("300 KB","do_increase_300"),("500 KB","do_increase_500"),("1 MB","do_increase_1024"),("2 MB","do_increase_2048")),
        R(("✏️ Custom KB","do_increase_custom")),
        R(BACK),
    ])

def crop_menu():
    return KB([
        R(("⬛ Square 1:1","do_crop_square"),("📱 Story 9:16","do_crop_story"),("🖼 Post 4:5","do_crop_post")),
        R(("🖥 Wide 16:9","do_crop_wide"),("🐦 Twitter 2:1","do_crop_twitter"),("📐 Custom ratio","do_ratio_custom")),
        R(BACK),
    ])

def rotate_menu():
    return KB([
        R(("↻ 90° CW","do_rotate_90"),("↺ 90° CCW","do_rotate_270"),("🔄 180°","do_rotate_180")),
        R(("↕️ Flip V","do_flip_v"),("↔️ Flip H","do_flip_h")),
        R(BACK),
    ])

def effects_menu():
    return KB([
        R(("⚫ Grayscale","do_fx_grayscale"),("🟤 Sepia","do_fx_sepia"),("💨 Blur","do_fx_blur")),
        R(("🔪 Sharpen","do_fx_sharpen"),("☀️ Bright+","do_fx_brightness_up"),("🌙 Bright-","do_fx_brightness_down")),
        R(("🎨 Contrast+","do_fx_contrast_up"),("📉 Contrast-","do_fx_contrast_down"),("🌈 Vivid","do_fx_vivid")),
        R(("❄️ Cool","do_fx_cool"),("🌅 Warm","do_fx_warm"),("🎞 Grain","do_fx_grain")),
        R(BACK),
    ])

def collage_menu():
    return KB([
        R(("▦ Grid 2×2","do_collage_grid"),("▶▶ Horizontal","do_collage_horizontal"),("▼▼ Vertical","do_collage_vertical")),
        R(BACK),
    ])

def batch_menu():
    return KB([
        R(("📦 Start collecting","do_batch_start")),
        R(("🔄 Batch resize","do_batch_resize"),("🗜 Batch compress","do_batch_compress"),("⚫ Batch grayscale","do_batch_grayscale")),
        R(("🖼 Batch collage","do_batch_collage")),
        R(BACK),
    ])

def pdf_rotate_menu():
    return KB([
        R(("↻ 90° CW","do_pdf_rotate_90"),("↺ 90° CCW","do_pdf_rotate_270"),("🔄 180°","do_pdf_rotate_180")),
        R(BACK),
    ])

# ═══════════════════════════════════════════════════════════
#  SEND HELPERS
# ═══════════════════════════════════════════════════════════
async def show_main(chat_id, ctx, caption="Choose what to do:"):
    await ctx.bot.send_message(chat_id, caption, reply_markup=image_menu())

async def send_doc(ctx, chat_id, buf, fname, cap="✅ Done!"):
    await ctx.bot.send_document(chat_id, buf, filename=fname, caption=cap)

# ═══════════════════════════════════════════════════════════
#  COMMAND HANDLERS
# ═══════════════════════════════════════════════════════════
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "👋 *MediaBot Pro*\n\nSend me an image, PDF, video, or a social media link.\n"
        "Every tool is shown upfront — no hidden menus.",
        parse_mode="Markdown", reply_markup=image_menu()
    )

async def help_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🛠 *All features:*\n"
        "Image: resize, convert, compress, increase size, crop, rotate/flip, effects, watermark, remove background, OCR, EXIF, auto color, border, collage, batch\n"
        "PDF: compress, extract text, extract tables→CSV, PDF→JPG, image→PDF, merge, split, encrypt, decrypt, rotate, watermark, info\n"
        "Other: video→GIF, social media downloader, QR code generator",
        parse_mode="Markdown"
    )

# ═══════════════════════════════════════════════════════════
#  MESSAGE HANDLERS
# ═══════════════════════════════════════════════════════════
async def on_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    fid = update.message.photo[-1].file_id
    cid = update.message.chat_id

    if ctx.user_data.get("batch_mode"):
        ctx.user_data.setdefault("batch_files",[]).append(fid)
        n = len(ctx.user_data["batch_files"])
        if n >= MAX_BATCH:
            await update.message.reply_text(f"✅ {n} images collected. Choose batch operation:")
            await ctx.bot.send_message(cid,"",reply_markup=batch_menu())
            ctx.user_data["batch_mode"] = False
        else:
            await update.message.reply_text(f"📦 Image {n}/{MAX_BATCH} collected. Send more or pick operation:")
            await ctx.bot.send_message(cid,"",reply_markup=batch_menu())
        return

    if state(ctx) == S.COLLAGE:
        ctx.user_data.setdefault("collage_files",[]).append(fid)
        n = len(ctx.user_data["collage_files"])
        await update.message.reply_text(f"🖼 {n} image(s) collected. Send more, or choose layout:")
        await ctx.bot.send_message(cid,"",reply_markup=collage_menu())
        return

    ctx.user_data["img_id"] = fid
    ctx.user_data.pop("pdf_id", None)
    clear_state(ctx)
    await update.message.reply_text("✅ Image received!", reply_markup=image_menu())

async def on_video(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if state(ctx) == S.VIDEO_GIF:
        vid = update.message.video
        await update.message.reply_text("⏳ Converting to GIF…")
        f = await ctx.bot.get_file(vid.file_id)
        vp = f"tmp_v_{update.message.chat_id}.mp4"
        gp = f"tmp_g_{update.message.chat_id}.gif"
        await f.download_to_drive(vp)
        try:
            await video_to_gif(vp, gp)
            with open(gp,"rb") as ff:
                await update.message.reply_animation(ff, caption="✅ GIF ready!")
        except Exception as e:
            await update.message.reply_text(f"❌ Failed: {str(e)[:100]}")
        finally:
            for p in [vp,gp]:
                try: os.remove(p)
                except: pass
            clear_state(ctx)
        return
    ctx.user_data["vid_id"] = update.message.video.file_id
    await update.message.reply_text(
        "🎬 Video received!",
        reply_markup=KB([R(("🎬 Convert to GIF","do_video_gif_prompt"))])
    )

async def on_doc(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    doc = update.message.document; cid = update.message.chat_id

    if state(ctx) == S.PDF_MERGE and doc.mime_type == "application/pdf":
        ctx.user_data.setdefault("merge_list",[]).append(doc.file_id)
        n = len(ctx.user_data["merge_list"])
        await update.message.reply_text(f"📎 PDF {n} added. Send another or type *done* to merge.", parse_mode="Markdown")
        return

    if doc.mime_type and doc.mime_type.startswith("image"):
        ctx.user_data["img_id"] = doc.file_id
        ctx.user_data.pop("pdf_id",None)
        await update.message.reply_text("✅ Image received!", reply_markup=image_menu())
    elif doc.mime_type == "application/pdf":
        ctx.user_data["pdf_id"] = doc.file_id
        ctx.user_data.pop("img_id",None)
        sz = (doc.file_size or 0)//1024
        await update.message.reply_text(f"📄 PDF received ({sz} KB)! All tools are shown above.", reply_markup=image_menu())
    else:
        await update.message.reply_text("⚠️ Send an image or PDF.")

async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    txt = update.message.text.strip()
    cid = update.message.chat_id
    s = state(ctx)

    # Social media download
    if is_sm_link(txt) and not s:
        await update.message.reply_text("📥 Downloading…")
        try:
            res = await dl_media(txt)
            with open(res["path"],"rb") as f:
                try: await update.message.reply_video(f, caption=f"✅ {res['title']}")
                except: await update.message.reply_document(f, caption=f"✅ {res['title']}")
            os.remove(res["path"])
        except Exception as e:
            err=str(e)
            if "403" in err or "blocked" in err.lower():
                await update.message.reply_text("❌ Platform is blocking downloads. Download manually and send me the file.")
            else:
                await update.message.reply_text(f"❌ Failed: {err[:150]}")
        return

    # PDF merge 'done'
    if s == S.PDF_MERGE and txt.lower() == "done":
        lst = ctx.user_data.get("merge_list",[])
        if len(lst) < 2:
            await update.message.reply_text("❌ Need at least 2 PDFs. Send more files.")
            return
        await update.message.reply_text(f"⏳ Merging {len(lst)} PDFs…")
        try:
            bl = [await get_bytes(ctx,fid) for fid in lst]
            buf = merge_pdfs(bl)
            await send_doc(ctx, cid, buf, "merged.pdf", f"✅ Merged {len(lst)} PDFs!")
        except Exception as e:
            await update.message.reply_text(f"❌ {str(e)[:100]}")
        finally:
            clear_state(ctx); ctx.user_data["merge_list"]=[]
        return

    img_id = ctx.user_data.get("img_id")
    pdf_id = ctx.user_data.get("pdf_id")

    if s == S.COMPRESS_KB:
        try:
            kb=int(txt); assert 1<=kb<=50000
            img=await get_img(ctx,img_id); buf=compress_to_kb(img,kb)
            actual=buf.seek(0,2)//1024; buf.seek(0)
            await send_doc(ctx,cid,buf,f"compressed_{kb}kb.jpg",f"✅ ~{actual} KB")
        except: await update.message.reply_text("❌ Enter a number 1–50000")
        finally: clear_state(ctx)
        return

    if s == S.INCREASE_KB:
        try:
            kb=int(txt); assert 1<=kb<=200000
            img=await get_img(ctx,img_id); buf=increase_to_kb(img,kb)
            actual=buf.seek(0,2)//1024; buf.seek(0)
            await send_doc(ctx,cid,buf,f"increased_{kb}kb.png",f"✅ ~{actual} KB")
        except: await update.message.reply_text("❌ Enter a number 1–200000")
        finally: clear_state(ctx)
        return

    if s == S.CUSTOM_SIZE:
        try:
            parts=re.split(r"[x×,\s]+",txt.lower()); w,h=int(parts[0]),int(parts[1])
            assert 1<=w<=10000 and 1<=h<=10000
            img=await get_img(ctx,img_id); img=img.resize((w,h),Image.LANCZOS)
            await send_doc(ctx,cid,img_buf(img),f"resized_{w}x{h}.png",f"✅ {w}×{h}")
        except: await update.message.reply_text("❌ Format: 800x600")
        finally: clear_state(ctx)
        return

    if s == S.CUSTOM_RATIO:
        try:
            parts=re.split(r"[:\s]+",txt); rw,rh=int(parts[0]),int(parts[1]); assert rw>0 and rh>0
            img=await get_img(ctx,img_id); img=crop_ratio(img,None)  # won't reach here
            # manual ratio crop
            ow,oh=img.size; tr=rw/rh; orr=ow/oh
            if orr>tr:
                nw=int(oh*tr); l=(ow-nw)//2; img=img.crop((l,0,l+nw,oh))
            else:
                nh=int(ow/tr); t=(oh-nh)//2; img=img.crop((0,t,ow,t+nh))
            await send_doc(ctx,cid,img_buf(img),f"ratio_{rw}x{rh}.png",f"✅ Cropped {rw}:{rh} → {img.size[0]}×{img.size[1]}")
        except: await update.message.reply_text("❌ Format: 16:9")
        finally: clear_state(ctx)
        return

    if s == S.PERCENT:
        try:
            p=int(txt); assert 1<=p<=500
            img=await get_img(ctx,img_id); nw,nh=int(img.width*p/100),int(img.height*p/100)
            img=img.resize((nw,nh),Image.LANCZOS)
            await send_doc(ctx,cid,img_buf(img),f"resized_{p}pct.png",f"✅ {p}% → {nw}×{nh}")
        except: await update.message.reply_text("❌ Enter 1–500")
        finally: clear_state(ctx)
        return

    if s == S.IMG_WM:
        try:
            img=await get_img(ctx,img_id); img=apply_watermark(img,txt)
            await send_doc(ctx,cid,img_buf(img),"watermarked.png","✅ Watermark added!")
        except Exception as e: await update.message.reply_text(f"❌ {e}")
        finally: clear_state(ctx)
        return

    if s == S.PDF_ENCRYPT:
        try:
            raw=await get_bytes(ctx,pdf_id); buf=encrypt_pdf(raw,txt)
            await send_doc(ctx,cid,buf,"encrypted.pdf",f"✅ Encrypted! Password: `{txt}`")
        except Exception as e: await update.message.reply_text(f"❌ {e}")
        finally: clear_state(ctx)
        return

    if s == S.PDF_WM:
        try:
            raw=await get_bytes(ctx,pdf_id); buf=watermark_pdf(raw,txt)
            await send_doc(ctx,cid,buf,"watermarked.pdf","✅ Watermark added to PDF!")
        except Exception as e: await update.message.reply_text(f"❌ {e}")
        finally: clear_state(ctx)
        return

    if s == S.QR_TEXT:
        if not QR_OK:
            await update.message.reply_text("❌ qrcode library not installed on server.")
            clear_state(ctx); return
        try:
            import qrcode as qrc
            qr=qrc.QRCode(error_correction=qrc.constants.ERROR_CORRECT_H,box_size=10,border=4)
            qr.add_data(txt); qr.make(fit=True)
            qimg=qr.make_image(fill_color="black",back_color="white").convert("RGB")
            qimg=qimg.resize((400,400),Image.LANCZOS)
            await send_doc(ctx,cid,img_buf(qimg),"qrcode.png","✅ QR code ready!")
        except Exception as e: await update.message.reply_text(f"❌ {e}")
        finally: clear_state(ctx)
        return

    if s == S.BATCH_SIZE:
        try:
            parts=re.split(r"[x×,\s]+",txt.lower()); w,h=int(parts[0]),int(parts[1])
            fids=ctx.user_data.get("batch_files",[])
            await update.message.reply_text(f"⏳ Resizing {len(fids)} images to {w}×{h}…")
            results=[]
            for fid in fids:
                img=await get_img(ctx,fid); img=img.resize((w,h),Image.LANCZOS)
                results.append(img_buf(img,fname=f"r_{w}x{h}.png"))
            zb=io.BytesIO()
            with zipfile.ZipFile(zb,"w") as zf:
                for i,b in enumerate(results): zf.writestr(f"resized_{i+1}.png",b.getvalue())
            zb.seek(0)
            await send_doc(ctx,cid,zb,"batch_resized.zip",f"✅ {len(results)} images resized!")
        except: await update.message.reply_text("❌ Format: 800x600")
        finally: clear_state(ctx)
        return

    if s == S.BATCH_KB:
        try:
            kb=int(txt)
            fids=ctx.user_data.get("batch_files",[])
            await update.message.reply_text(f"⏳ Compressing {len(fids)} images to {kb} KB…")
            zb=io.BytesIO()
            with zipfile.ZipFile(zb,"w") as zf:
                for i,fid in enumerate(fids):
                    img=await get_img(ctx,fid); b=compress_to_kb(img,kb)
                    zf.writestr(f"compressed_{i+1}.jpg",b.getvalue())
            zb.seek(0)
            await send_doc(ctx,cid,zb,"batch_compressed.zip",f"✅ {len(fids)} images compressed!")
        except: await update.message.reply_text("❌ Enter a number")
        finally: clear_state(ctx)
        return

    await update.message.reply_text("❓ Send an image, PDF, video, or social link. Use /start to see the menu.")

# ═══════════════════════════════════════════════════════════
#  BUTTON DISPATCHER
# ═══════════════════════════════════════════════════════════
async def button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q=update.callback_query; await q.answer()
    d=q.data; cid=q.message.chat_id
    img_id=ctx.user_data.get("img_id")
    pdf_id=ctx.user_data.get("pdf_id")

    # ── noop (section dividers) ───────────────────────────
    if d=="noop": return

    # ── Sub-menus ─────────────────────────────────────────
    MENUS = {
        "menu_main":       ("Choose what to do:", image_menu()),
        "menu_img_resize": ("📐 Resize:", resize_menu()),
        "menu_img_convert":("🔄 Convert to:", convert_menu()),
        "menu_img_compress":("🗜 Compress to:", compress_menu()),
        "menu_img_increase":("📈 Increase to:", increase_menu()),
        "menu_img_crop":   ("✂️ Crop preset:", crop_menu()),
        "menu_img_rotate": ("🔄 Rotate / Flip:", rotate_menu()),
        "menu_img_effects":("🎨 Effects:", effects_menu()),
        "menu_collage":    ("🖼 Collage layout:", collage_menu()),
        "menu_batch":      ("📦 Batch operations:", batch_menu()),
        "menu_pdf_rotate": ("🔄 Rotate PDF pages:", pdf_rotate_menu()),
    }
    if d in MENUS:
        txt, kb = MENUS[d]
        await q.edit_message_text(txt, reply_markup=kb)
        return

    # ── Prompts that need text input ──────────────────────
    if d=="do_compress_custom":
        set_state(ctx,S.COMPRESS_KB)
        await q.edit_message_text("✏️ Enter target size in KB (e.g. 150):"); return
    if d=="do_increase_custom":
        set_state(ctx,S.INCREASE_KB)
        await q.edit_message_text("✏️ Enter target size in KB (e.g. 800):"); return
    if d=="do_resize_custom":
        set_state(ctx,S.CUSTOM_SIZE)
        await q.edit_message_text("✏️ Enter size (e.g. 800x600):"); return
    if d=="do_ratio_custom":
        set_state(ctx,S.CUSTOM_RATIO)
        await q.edit_message_text("✏️ Enter ratio (e.g. 16:9):"); return
    if d=="do_percent_prompt":
        set_state(ctx,S.PERCENT)
        await q.edit_message_text("✏️ Enter percentage (e.g. 75 or 150):"); return
    if d=="do_wm_prompt":
        set_state(ctx,S.IMG_WM)
        await q.edit_message_text("✏️ Enter watermark text:"); return
    if d=="do_video_gif_prompt":
        set_state(ctx,S.VIDEO_GIF)
        await q.edit_message_text("🎬 Send me the video file (MP4, max 8s clip will be used)."); return
    if d=="do_qr_prompt":
        set_state(ctx,S.QR_TEXT)
        await q.edit_message_text("🔲 Enter text or URL for QR code:"); return
    if d=="do_dl_prompt":
        await q.edit_message_text("📥 Send me a Pinterest, Instagram, TikTok, YouTube Shorts, or Twitter/X link."); return
    if d=="do_pdf_merge_start":
        set_state(ctx,S.PDF_MERGE); ctx.user_data["merge_list"]=[]
        await q.edit_message_text("🔗 Send PDFs one by one. Type *done* when finished.", parse_mode="Markdown"); return
    if d=="do_pdf_encrypt":
        set_state(ctx,S.PDF_ENCRYPT)
        await q.edit_message_text("🔒 Enter password to encrypt the PDF:"); return
    if d=="do_pdf_wm_prompt":
        set_state(ctx,S.PDF_WM)
        await q.edit_message_text("✏️ Enter watermark text for PDF:"); return

    # ── Collage collection ────────────────────────────────
    if d in ("do_collage_grid","do_collage_horizontal","do_collage_vertical"):
        mode=d.split("_")[-1]
        fids=ctx.user_data.get("collage_files",[])
        if len(fids)<2:
            set_state(ctx,S.COLLAGE); ctx.user_data["collage_files"]=[]
            ctx.user_data["collage_mode"]=mode
            await q.edit_message_text("🖼 Send me 2–6 images. I'll collage them when you pick a layout again."); return
        await q.edit_message_text(f"⏳ Building {mode} collage…")
        try:
            imgs=[await get_img(ctx,fid) for fid in fids]
            buf=make_collage(imgs,mode)
            await send_doc(ctx,cid,buf,"collage.jpg","✅ Collage ready!")
        except Exception as e: await ctx.bot.send_message(cid,f"❌ {e}")
        finally: clear_state(ctx); ctx.user_data["collage_files"]=[]
        await show_main(cid,ctx); return

    # ── Batch ops ─────────────────────────────────────────
    if d=="do_batch_start":
        ctx.user_data["batch_mode"]=True; ctx.user_data["batch_files"]=[]
        await q.edit_message_text(f"📦 Batch mode on. Send up to {MAX_BATCH} images, then choose an operation."); return

    if d=="do_batch_resize":
        if not ctx.user_data.get("batch_files"):
            await q.edit_message_text("❌ No images collected. Start batch mode first."); return
        set_state(ctx,S.BATCH_SIZE); await q.edit_message_text("✏️ Enter target size (e.g. 800x600):"); return

    if d=="do_batch_compress":
        if not ctx.user_data.get("batch_files"):
            await q.edit_message_text("❌ No images collected. Start batch mode first."); return
        set_state(ctx,S.BATCH_KB); await q.edit_message_text("✏️ Enter target KB:"); return

    if d=="do_batch_grayscale":
        fids=ctx.user_data.get("batch_files",[])
        if not fids: await q.edit_message_text("❌ No images in batch."); return
        await q.edit_message_text(f"⏳ Converting {len(fids)} images to grayscale…")
        zb=io.BytesIO()
        with zipfile.ZipFile(zb,"w") as zf:
            for i,fid in enumerate(fids):
                img=await get_img(ctx,fid); img=img.convert("L")
                b=io.BytesIO(); img.save(b,"PNG"); zf.writestr(f"gray_{i+1}.png",b.getvalue())
        zb.seek(0); await send_doc(ctx,cid,zb,"batch_grayscale.zip",f"✅ {len(fids)} images done!")
        await show_main(cid,ctx); return

    if d=="do_batch_collage":
        fids=ctx.user_data.get("batch_files",[])
        if len(fids)<2: await q.edit_message_text("❌ Need 2+ images in batch."); return
        await q.edit_message_text("⏳ Building collage from batch…")
        imgs=[await get_img(ctx,fid) for fid in fids]
        buf=make_collage(imgs,"grid")
        await send_doc(ctx,cid,buf,"batch_collage.jpg","✅ Batch collage ready!")
        await show_main(cid,ctx); return

    # ── Image checks ──────────────────────────────────────
    IMAGE_CBS = (
        d.startswith("do_resize_") or d.startswith("do_compress_") or
        d.startswith("do_increase_") or d.startswith("do_convert_") or
        d.startswith("do_fx_") or d.startswith("do_rotate_") or
        d.startswith("do_flip_") or d.startswith("do_crop_") or
        d in ("do_img_info","do_auto_color","do_remove_exif","do_add_border","do_remove_bg","do_ocr","do_exif","do_img2pdf")
    )
    if IMAGE_CBS and not img_id:
        await q.edit_message_text("❌ No image loaded. Send an image first."); return

    # ── PDF checks ────────────────────────────────────────
    PDF_CBS = (
        d.startswith("do_pdf_") or d in ("do_pdf2jpg",)
    )
    if PDF_CBS and not pdf_id:
        await q.edit_message_text("❌ No PDF loaded. Send a PDF file first."); return

    # ── Image info ────────────────────────────────────────
    if d=="do_img_info":
        img=await get_img(ctx,img_id)
        try:
            from PIL.ExifTags import TAGS
            ex=img._getexif()
            exif_str=""
            if ex:
                bits=[]
                for tid,val in list(ex.items())[:8]:
                    if not isinstance(val,bytes): bits.append(f"{TAGS.get(tid,tid)}: {str(val)[:40]}")
                exif_str="\n".join(bits)
        except: exif_str=""
        txt=(f"📊 *Image Info*\n"
             f"Size: {img.width}×{img.height} px\n"
             f"Mode: {img.mode}\n"
             f"Format: {img.format or 'memory stream'}\n"
             + (f"\n*EXIF:*\n{exif_str}" if exif_str else ""))
        await q.edit_message_text(txt, parse_mode="Markdown", reply_markup=image_menu()); return

    # ── Resize presets ────────────────────────────────────
    if d.startswith("do_resize_"):
        parts=d.split("_"); w,h=int(parts[2]),int(parts[3])
        img=await get_img(ctx,img_id); img=img.resize((w,h),Image.LANCZOS)
        await q.edit_message_text(f"⏳ Resizing to {w}×{h}…")
        await send_doc(ctx,cid,img_buf(img),f"resized_{w}x{h}.png",f"✅ {w}×{h}")
        await show_main(cid,ctx); return

    # ── Compress presets ──────────────────────────────────
    if d.startswith("do_compress_"):
        kb=int(d.split("_")[-1])
        await q.edit_message_text(f"⏳ Compressing to {kb} KB…")
        img=await get_img(ctx,img_id); buf=compress_to_kb(img,kb)
        actual=buf.seek(0,2)//1024; buf.seek(0)
        await send_doc(ctx,cid,buf,f"compressed_{kb}kb.jpg",f"✅ ~{actual} KB")
        await show_main(cid,ctx); return

    # ── Increase presets ──────────────────────────────────
    if d.startswith("do_increase_"):
        kb=int(d.split("_")[-1])
        await q.edit_message_text(f"⏳ Increasing to {kb} KB…")
        img=await get_img(ctx,img_id); buf=increase_to_kb(img,kb)
        actual=buf.seek(0,2)//1024; buf.seek(0)
        await send_doc(ctx,cid,buf,f"increased_{kb}kb.png",f"✅ ~{actual} KB")
        await show_main(cid,ctx); return

    # ── Crop presets ──────────────────────────────────────
    if d.startswith("do_crop_"):
        preset=d[len("do_crop_"):]
        img=await get_img(ctx,img_id); img=crop_ratio(img,preset)
        await send_doc(ctx,cid,img_buf(img),f"crop_{preset}.png",f"✅ Cropped to {preset} ratio")
        await show_main(cid,ctx); return

    # ── Rotate / flip ─────────────────────────────────────
    if d.startswith("do_rotate_"):
        angle=int(d.split("_")[-1])
        img=await get_img(ctx,img_id); img=img.rotate(angle,expand=True)
        await send_doc(ctx,cid,img_buf(img),f"rotated_{angle}.png",f"✅ Rotated {angle}°")
        await show_main(cid,ctx); return
    if d=="do_flip_v":
        img=await get_img(ctx,img_id); img=img.transpose(Image.FLIP_TOP_BOTTOM)
        await send_doc(ctx,cid,img_buf(img),"flipped_v.png","✅ Flipped vertically"); await show_main(cid,ctx); return
    if d=="do_flip_h":
        img=await get_img(ctx,img_id); img=img.transpose(Image.FLIP_LEFT_RIGHT)
        await send_doc(ctx,cid,img_buf(img),"flipped_h.png","✅ Flipped horizontally"); await show_main(cid,ctx); return

    # ── Effects ───────────────────────────────────────────
    if d.startswith("do_fx_"):
        fx=d[len("do_fx_"):]; await q.edit_message_text("⏳ Applying effect…")
        img=await get_img(ctx,img_id)
        if fx=="grayscale": img=img.convert("L")
        elif fx=="sepia":
            img=img.convert("RGB"); w,h=img.size; px=img.load()
            for x in range(w):
                for y in range(h):
                    r,g,b=px[x,y]
                    px[x,y]=(min(int(0.393*r+0.769*g+0.189*b),255),
                              min(int(0.349*r+0.686*g+0.168*b),255),
                              min(int(0.272*r+0.534*g+0.131*b),255))
        elif fx=="blur": img=img.filter(ImageFilter.BLUR)
        elif fx=="sharpen": img=img.filter(ImageFilter.SHARPEN)
        elif fx=="brightness_up": img=ImageEnhance.Brightness(img).enhance(1.5)
        elif fx=="brightness_down": img=ImageEnhance.Brightness(img).enhance(0.6)
        elif fx=="contrast_up": img=ImageEnhance.Contrast(img).enhance(1.5)
        elif fx=="contrast_down": img=ImageEnhance.Contrast(img).enhance(0.6)
        elif fx=="vivid": img=ImageEnhance.Color(img).enhance(1.8)
        elif fx=="cool":
            img=img.convert("RGB"); r,g,b=img.split()
            r=r.point(lambda i:max(0,i-20)); b=b.point(lambda i:min(255,i+20))
            img=Image.merge("RGB",(r,g,b))
        elif fx=="warm":
            img=img.convert("RGB"); r,g,b=img.split()
            r=r.point(lambda i:min(255,i+20)); b=b.point(lambda i:max(0,i-15))
            img=Image.merge("RGB",(r,g,b))
        elif fx=="grain":
            import random; img=img.convert("RGB"); px=img.load()
            for x in range(img.width):
                for y in range(img.height):
                    n=random.randint(-18,18); r,g,b=px[x,y]
                    px[x,y]=(max(0,min(255,r+n)),max(0,min(255,g+n)),max(0,min(255,b+n)))
        await send_doc(ctx,cid,img_buf(img),f"{fx}.png",f"✅ {fx.replace('_',' ').title()} applied")
        await show_main(cid,ctx); return

    # ── Convert ───────────────────────────────────────────
    if d.startswith("do_convert_"):
        fmt=d.split("_")[-1]; await q.edit_message_text(f"⏳ Converting to {fmt}…")
        img=await get_img(ctx,img_id)
        await send_doc(ctx,cid,img_buf(img,fmt),f"converted.{fmt.lower()}",f"✅ Converted to {fmt}")
        await show_main(cid,ctx); return

    # ── Image → PDF ───────────────────────────────────────
    if d=="do_img2pdf":
        await q.edit_message_text("⏳ Converting image to PDF…")
        img=await get_img(ctx,img_id); img=img.convert("RGB")
        tmp=io.BytesIO(); img.save(tmp,"JPEG"); tmp.seek(0)
        buf=io.BytesIO(img2pdf.convert(tmp.read())); buf.seek(0)
        await send_doc(ctx,cid,buf,"converted.pdf","✅ PDF ready!")
        await show_main(cid,ctx); return

    # ── Misc image ops ────────────────────────────────────
    if d=="do_auto_color":
        img=await get_img(ctx,img_id); img=ImageEnhance.Color(img).enhance(1.3)
        await send_doc(ctx,cid,img_buf(img),"auto_color.png","✅ Auto color enhanced")
        await show_main(cid,ctx); return

    if d=="do_remove_exif":
        img=await get_img(ctx,img_id)
        clean=Image.new(img.mode,img.size); clean.putdata(list(img.getdata()))
        await send_doc(ctx,cid,img_buf(clean),"no_exif.png","✅ EXIF removed")
        await show_main(cid,ctx); return

    if d=="do_exif":
        img=await get_img(ctx,img_id)
        try:
            from PIL.ExifTags import TAGS
            ex=img._getexif(); lines=["📷 *Full EXIF*\n"]
            if ex:
                for tid,val in ex.items():
                    if isinstance(val,bytes): continue
                    lines.append(f"*{TAGS.get(tid,tid)}:* {str(val)[:60]}")
            txt="\n".join(lines) if len(lines)>1 else "No EXIF data found."
        except: txt="Could not read EXIF."
        await q.edit_message_text(txt[:4000], parse_mode="Markdown", reply_markup=image_menu()); return

    if d=="do_add_border":
        img=await get_img(ctx,img_id); B=20
        c=Image.new("RGB",(img.width+2*B,img.height+2*B),(255,255,255)); c.paste(img,(B,B))
        await send_doc(ctx,cid,img_buf(c),"bordered.png","✅ Border added")
        await show_main(cid,ctx); return

    if d=="do_remove_bg":
        if not REMBG_OK:
            await q.edit_message_text("❌ rembg not installed on this server."); return
        await q.edit_message_text("⏳ Removing background…")
        raw=await get_bytes(ctx,img_id)
        result=rembg_remove(raw)
        buf=io.BytesIO(result); buf.seek(0)
        await send_doc(ctx,cid,buf,"no_bg.png","✅ Background removed!")
        await show_main(cid,ctx); return

    if d=="do_ocr":
        if not OCR_OK:
            await q.edit_message_text("❌ pytesseract not installed on this server."); return
        await q.edit_message_text("⏳ Extracting text…")
        img=await get_img(ctx,img_id)
        txt=pytesseract.image_to_string(img).strip() or "No text found."
        buf=io.BytesIO(txt.encode()); buf.seek(0)
        await send_doc(ctx,cid,buf,"ocr_result.txt",f"✅ Text extracted! ({len(txt)} chars)")
        await show_main(cid,ctx); return

    # ══════════════════════════════════════════════════════
    #  PDF OPERATIONS
    # ══════════════════════════════════════════════════════

    if d=="do_pdf_compress":
        await q.edit_message_text("⏳ Compressing PDF…")
        try:
            raw=await get_bytes(ctx,pdf_id)
            orig_kb=len(raw)//1024
            buf=compress_pdf(raw)
            new_kb=buf.seek(0,2)//1024; buf.seek(0)
            await send_doc(ctx,cid,buf,"compressed.pdf",
                f"✅ Done!\nBefore: {orig_kb} KB → After: {new_kb} KB (saved {orig_kb-new_kb} KB)")
        except Exception as e:
            await ctx.bot.send_message(cid,f"❌ Failed: {str(e)[:150]}")
        await show_main(cid,ctx); return

    if d=="do_pdf_info":
        raw=await get_bytes(ctx,pdf_id)
        await q.edit_message_text(pdf_info(raw), parse_mode="Markdown", reply_markup=image_menu()); return

    if d=="do_pdf_text":
        await q.edit_message_text("⏳ Extracting text from PDF…")
        raw=await get_bytes(ctx,pdf_id)
        try:
            txt=extract_pdf_text(raw)
            buf=io.BytesIO(txt.encode()); buf.seek(0)
            await send_doc(ctx,cid,buf,"extracted_text.txt",f"✅ Text extracted!")
        except Exception as e:
            await ctx.bot.send_message(cid,f"❌ {e}")
        await show_main(cid,ctx); return

    if d=="do_pdf_tables":
        await q.edit_message_text("⏳ Extracting tables…")
        raw=await get_bytes(ctx,pdf_id)
        try:
            buf=extract_pdf_tables(raw)
            await send_doc(ctx,cid,buf,"tables.csv","✅ Tables extracted to CSV!")
        except Exception as e:
            await ctx.bot.send_message(cid,f"❌ {e}")
        await show_main(cid,ctx); return

    if d=="do_pdf2jpg":
        await q.edit_message_text("⏳ Converting PDF pages to JPG…")
        raw=await get_bytes(ctx,pdf_id)
        try:
            from pdf2image import convert_from_bytes
            pages=convert_from_bytes(raw,dpi=150)
            for i,pg in enumerate(pages,1):
                buf=io.BytesIO(); pg.save(buf,"JPEG",quality=90); buf.seek(0)
                await ctx.bot.send_document(cid,buf,filename=f"page_{i}.jpg")
        except Exception as e:
            await ctx.bot.send_message(cid,f"❌ {e}")
        await show_main(cid,ctx); return

    if d=="do_pdf_split":
        await q.edit_message_text("⏳ Splitting PDF…")
        raw=await get_bytes(ctx,pdf_id)
        pages=split_pdf(raw)
        if len(pages)<=10:
            for n,buf in pages: await ctx.bot.send_document(cid,buf,filename=f"page_{n}.pdf")
        else:
            zb=io.BytesIO()
            with zipfile.ZipFile(zb,"w") as zf:
                for n,buf in pages: zf.writestr(f"page_{n}.pdf",buf.getvalue())
            zb.seek(0); await send_doc(ctx,cid,zb,"split_pages.zip",f"✅ {len(pages)} pages split!")
        await show_main(cid,ctx); return

    if d=="do_pdf_decrypt":
        await q.edit_message_text("⏳ Removing encryption…")
        raw=await get_bytes(ctx,pdf_id)
        try:
            r=PdfReader(io.BytesIO(raw))
            if r.is_encrypted: r.decrypt("")
            w=PdfWriter()
            for pg in r.pages: w.add_page(pg)
            buf=io.BytesIO(); w.write(buf); buf.seek(0)
            await send_doc(ctx,cid,buf,"decrypted.pdf","✅ Decrypted! (works only for open-password PDFs)")
        except Exception as e:
            await ctx.bot.send_message(cid,f"❌ Cannot decrypt: {str(e)[:100]}")
        await show_main(cid,ctx); return

    if d.startswith("do_pdf_rotate_"):
        angle=int(d.split("_")[-1]); await q.edit_message_text(f"⏳ Rotating {angle}°…")
        raw=await get_bytes(ctx,pdf_id)
        buf=rotate_pdf(raw,angle)
        await send_doc(ctx,cid,buf,f"rotated_{angle}.pdf",f"✅ All pages rotated {angle}°!")
        await show_main(cid,ctx); return

    await q.edit_message_text("❓ Unknown action.", reply_markup=image_menu())

# ═══════════════════════════════════════════════════════════
#  MAIN
# ═══════════════════════════════════════════════════════════
if __name__=="__main__":
    os.makedirs("downloads",exist_ok=True)
    app=ApplicationBuilder().token(TOKEN).build()
    app.add_handler(CommandHandler("start",start))
    app.add_handler(CommandHandler("help",help_cmd))
    app.add_handler(CallbackQueryHandler(button))
    app.add_handler(MessageHandler(filters.PHOTO,on_photo))
    app.add_handler(MessageHandler(filters.VIDEO,on_video))
    app.add_handler(MessageHandler(filters.Document.ALL,on_doc))
    app.add_handler(MessageHandler(filters.TEXT&~filters.COMMAND,on_text))
    print("✅ MediaBot Pro started.")
    app.run_polling()
